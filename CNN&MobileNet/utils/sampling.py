...
def cifar_noniid(dataset, num_users):
    ...
    idx_shard = [i for i in range(50000)]
    dict_users = {i: np.array([]) for i in range(num_users)}
    ...
    # divide and assign
    num_main_class = 100  #5 users 4000  ## 10 users: 4000 # 500
    num_other_class = int(50000/num_users) - num_main_class #10000 - 2*num_main_class #5 users
    rand_class = 9
    temp_point=5000
    for i in range(num_users): #choose main class
        rand_set = set(np.random.choice(idx_shard[(temp_point*rand_class):temp_point*(rand_class +1)], num_main_class, replace=False))
        idx_shard = list(set(idx_shard) - rand_set)
        for j in rand_set:
            dict_users[i] = np.append(dict_users[i], idxs[j])
        rand_class -= 1
        if rand_class<0:
            rand_class=rand_class+10
            temp_point=temp_point-num_main_class
        #
        # rand_set2 = set(
